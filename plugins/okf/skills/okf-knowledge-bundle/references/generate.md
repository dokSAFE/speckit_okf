...
4. Note the **git history signals** under `git.history` in the inventory:
   ...
   - `untracked_dirs` — directories present on disk that git does not
     ...
     in your report instead and ask whether any should be documented
     separately; if so, the fix is to track the subtree or catalog it as
     its own repository, not to fold it into this bundle.
...
```bash
python3 $SKILL_DIR/scripts/python/okf-cochange.py --depth 2          # repo-wide: most coupled units
python3 $SKILL_DIR/scripts/python/okf-cochange.py <dir> --depth 3    # what moves when this moves
```
...
(`](/services/payment.md)`), because a leading `/` survives a document moving
within its subdirectory. Decline that here: a leading `/` resolves against the
**bundle** root for OKF and the **repository** root for GitHub and every other
forge. Whenever the bundle sits in a subdirectory — the normal case — every
such link 404s in a browser while validating perfectly. The validator reports
it as **W9**. If the bundle *is* the repository root, either form works.
...
Keep it short and aimed at a human arriving cold: what the directory is and
that it is generated; a link straight to `architecture/overview.md` and one to
`index.md`; concept count, generator version and originating commit; how to
check it with `verify_okf.py`; and **how to correct it** — that uncertainty is
parked in `open_questions`, that the **clarify** workflow walks a human
through them, and that an answer given there is sentinel-marked and survives
every later regeneration. That is the highest-value thing a reader can do with
the bundle and the only part a machine cannot produce, so say it plainly.
...
3. **Verify the claims, not just the structure:**
   ...
   - **V3** — you cited a `[TEST-ONLY]` commit as a gotcha. Delete the claim.
   ...
   NOTEs need judgement rather than obedience: **V6** is often a real runtime
   coupling, and **V7** means you described Go packages as an import cycle,
   which the compiler forbids.
